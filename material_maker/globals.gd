...
func popup_menu(menu : PopupMenu, parent : Control):
	...
	menu.get_window().content_scale_factor = csf
	menu.position = parent.get_local_mouse_position()*csf*zoom_fac + parent.get_screen_position()
	menu.get_window().transient = true
	menu.show()
